...
/// Runs the `kickbacks` CLI's `model` command and decodes its JSON.
///
...
public enum ModelClient {
  /// Resolve the kickbacks binary: $KICKBACKS_BIN, then the common Homebrew paths.
  public static func binaryPath() -> String? {
    if let b = ProcessInfo.processInfo.environment["KICKBACKS_BIN"], !b.isEmpty { return b }
    // Bundled next to the app's own executable (Kickbacks.app/Contents/MacOS/kickbacks) →
    // makes the .app self-contained, no brew required.
    if let exe = Bundle.main.executableURL {
      let sibling = exe.deletingLastPathComponent().appendingPathComponent("kickbacks").path
      if FileManager.default.isExecutableFile(atPath: sibling) { return sibling }
    }
    for p in ["/opt/homebrew/bin/kickbacks", "/usr/local/bin/kickbacks"]
  ...
  public static func pollerInstalled() -> Bool {
    let path = (NSHomeDirectory() as NSString).appendingPathComponent("Library/LaunchAgents/ai.kickbacks.poller.plist")
